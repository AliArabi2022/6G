function nDrawn = simple_boxplot(ax, dataCell, xPositions, boxWidth, faceColor)
...
%
% OUTPUT
%   nDrawn     - number of boxes actually drawn (i.e., had >=1 finite,
%                real data point). Callers should check this: if it's 0
%                despite dataCell having valid-looking data, something
%                upstream of this function is wrong, not this function.
%
...
% Date: 2026-07-07
    ...
    % NOTE: deliberately NOT calling axes(ax) here. Every graphics call
    % below (patch(ax,...), plot(ax,...)) already explicitly targets ax
    % as its first argument, which is the modern/recommended MATLAB
    % pattern and does not depend on "current axes" state at all. Calling
    % axes(ax) first is redundant at best; at worst, in some MATLAB
    % versions/configurations, forcing a figure/axes to become "current"
    % can interact unexpectedly with figure focus, uifigure-vs-figure
    % contexts, or automated/non-interactive rendering. Removing it
    % eliminates that entire class of possible issues.
    hold(ax, 'on');
    nDrawn = 0;
