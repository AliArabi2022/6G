...
for s = 1:numel(scenarios)
    ...
    for kk = 1:numel(Kvals)
        ...
        dataCell = {to_dB_safe(results.(sc.name).g_coh{kk}), to_dB_safe(results.(sc.name).g_opt{kk})};
        nValidCoh = sum(isfinite(dataCell{1}));
        nValidOpt = sum(isfinite(dataCell{2}));
        fprintf('    [%s, K=%d] valid points for plotting: coherent=%d/%d, optimal=%d/%d\n', ...
            sc.name, K, nValidCoh, nRealizations, nValidOpt, nRealizations);
        if nValidCoh == 0
        ...
        colors = {[0.85 0.45 0.15], [0.20 0.45 0.80]}; % orange=coherent, blue=optimal
        if ~hasCVX
            dataCell = dataCell(1); colors = colors(1); % coherent-only fallback
            xPos = 1; xlim(ax, [0.5 1.5]); xticks(ax, 1); xticklabels(ax, {'Coh'});
        else
            xPos = [1 2]; xlim(ax, [0.5 2.5]); xticks(ax, [1 2]); xticklabels(ax, {'Coh','Opt'});
        end
        simple_boxplot(ax, dataCell, xPos, 0.4, colors);
        grid(ax, 'on');
